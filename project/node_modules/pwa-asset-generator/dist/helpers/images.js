...
const getSplashScreenImages = (uniformSplashScreenData, options) => {
    ...
    return lodash_uniqwith_1.default(uniformSplashScreenData.reduce((acc, curr) => {
        let images = acc;
...
const getSplashScreenScaleFactorUnionData = (launchScreenSpecs, deviceFactorSpecs) => {
    return launchScreenSpecs.map((launchScreenSpec) => {
        const matchedDevice = deviceFactorSpecs.find((deviceFactorSpec) => deviceFactorSpec.device === launchScreenSpec.device);
        if (matchedDevice) {
            return Object.assign(Object.assign({}, launchScreenSpec), { scaleFactor: matchedDevice.scaleFactor });
        }
        return launchScreenSpec;
    });
};
exports.default = {
    getIconImages,

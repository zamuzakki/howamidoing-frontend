...
const generateAppleTouchIconHtml = (savedImages, options) => {
    return savedImages
        ...
        .map(({ width, path, name }) => constants_1.default.APPLE_TOUCH_ICON_META_HTML(width, generateOutputPath(options, name, path)))
        .join('');
};
const generateFaviconHtml = (savedImages, options) => {
    return savedImages
        ...
        .map(({ width, path, name }) => constants_1.default.FAVICON_META_HTML(width, generateOutputPath(options, name, path), mime_types_1.lookup(path)))
        .join('');
};
const generateAppleLaunchImageHtml = (savedImages, options, darkMode) => {
    return savedImages
        ...
        .map(({ width, height, path, name, scaleFactor, orientation }) => constants_1.default.APPLE_LAUNCH_SCREEN_META_HTML(width, height, generateOutputPath(options, name, path), scaleFactor, orientation, darkMode))
        .join('');
};
const generateHtmlForIndexPage = (savedImages, options) => {
    const htmlMeta = {
        [meta_1.HTMLMetaNames.appleMobileWebAppCapable]: `<meta name="apple-mobile-web-app-capable" content="yes">
`,
    };
...
const addMetaTagsToIndexPage = (htmlMeta, indexHtmlFilePath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const $ = cheerio_1.default.load(indexHtmlFile, { decodeEntities: false });

        ...
        const scrapeSplashScreenDataFromHIGPage = () => {
            ...
            return Array.from((_a = document.querySelectorAll('table')) === null || _a === void 0 ? void 0 : _a[0].querySelectorAll('tbody tr')).map(tr => {
                ...
                (acc, curr, index) => {
                    ...
                    return Object.assign(Object.assign({}, acc), { [keyToUpdate]: index > 0
                            ? getDimensions(curr.innerText)
                            : curr.innerText });
                }, {
                    device: '',
...
const getDeviceScaleFactorData = (browser, options) => __awaiter(void 0, void 0, void 0, function* () {
    const logger = logger_1.default(getDeviceScaleFactorData.name, options);
    const page = yield browser.newPage();
    yield page.setUserAgent(constants_1.default.EMULATED_USER_AGENT);
    logger.log(`Navigating to Apple Human Interface Guidelines website - ${constants_1.default.APPLE_HIG_DEVICE_SCALE_FACTOR_SPECS_URL}`);
    yield page.goto(constants_1.default.APPLE_HIG_DEVICE_SCALE_FACTOR_SPECS_URL, {
        waitUntil: 'networkidle0',
    });
    try {
        yield page.waitForSelector('table', {
        ...
        const err = `Could not find the table on the page within timeout ${constants_1.default.WAIT_FOR_SELECTOR_TIMEOUT}ms`;
        logger.error(err);
        throw Error(err);
    }
    const scaleFactorData = yield page.evaluate(({ selector }) => {
        // TypeScript doesn't recognize the page context within callback
        /* eslint-disable @typescript-eslint/ban-ts-ignore */
        const scrapeScaleFactorDataFromHIGPage = () => Array.from(document.querySelectorAll(selector)).map(tr => {
            return Array.from(tr.querySelectorAll('td')).reduce(
            // @ts-ignore
            (acc, curr, index) => {
                ...
                return Object.assign(Object.assign({}, acc), { [keyToUpdate]: index > 0
                        ? getScaleFactor(curr.innerText)
                        : curr.innerText });
            }, { device: '', scaleFactor: 1 });
        });
        /* eslint-enable @typescript-eslint/ban-ts-ignore */
        return scrapeScaleFactorDataFromHIGPage();
    }, { selector: constants_1.default.APPLE_HIG_SPLASH_SCR_SPECS_DATA_GRID_SELECTOR });
    if (!scaleFactorData.length) {
        const err = `Failed scraping the data on web page ${constants_1.default.APPLE_HIG_DEVICE_SCALE_FACTOR_SPECS_URL}`;
        logger.error(err);
        throw Error(err);
    }
...
const getSplashScreenMetaData = (options, browser) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const splashScreenData = yield getAppleSplashScreenData(browser, options);
        const scaleFactorData = yield getDeviceScaleFactorData(browser, options);
        splashScreenUniformMetaData = images_1.default.getSplashScreenScaleFactorUnionData(splashScreenData, scaleFactorData);
